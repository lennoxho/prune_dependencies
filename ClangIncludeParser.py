...
class IncludeParser:
    ...
    def get_includes(self, filename, include_dirs, args):
        ...
        def list_includes(tu):
            ...
            for child in cursor.get_children():
                if (child.kind == clang.cindex.CursorKind.INCLUSION_DIRECTIVE and
                    ...
                    if not self.__strict or (reg.group(1) == "<" and os.path.basename(include) == include):
                        if not self.__strict and reg.group(1) == '"':
        ...
        tu = self.__index.parse(filename, args, None, flags)
